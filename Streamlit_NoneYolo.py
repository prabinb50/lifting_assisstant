    ...
    if results_pose.pose_landmarks:
        ...
        try:
            ...
            if "down" in exercise_class:
                ...
                #     # If no specific issue was found, mark as correct form
                #     if not found_issue:
                #         results['posture_issue'] = "correct"
...
with tab2:
    ...
    #     with col3:
    #         most_common_issue = most_frequent([i for i in posture_issues if i != "correct" and i is not None]) or "None"
    ...
    if st.session_state.video_processed and st.session_state.video_results:
        ...
        # Posture issues summary
        posture_issues = [result['posture_issue'] for result in st.session_state.video_results 
                        if result['posture_issue'] is not None and result['rep_counted']]
        
        with col2:
            # Fix for correct form calculation
            total_completed_reps = st.session_state.counter
            
            # Count reps where no issues were found
            correct_reps = sum(1 for issue in posture_issues if issue == "correct")
            
            # Calculate percentage of correct reps
            if total_completed_reps > 0:
                correct_percentage = (correct_reps / total_completed_reps) * 100
            else:
                correct_percentage = 0.0
        ...
        with col3:
            # Find most common issue excluding "correct"
